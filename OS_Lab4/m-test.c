 ...
 * there is the same form of race condition_sat between the threads, such that 
 * standard output of odd and even numbers is unpredictably interspersed.
 *
 */
bool check = false;

pthread_mutex_t thread_race_lock;

pthread_cond_t condition_sat;
/**
...
void* print_odds(void* arg) {
   ...
   for (i = 1; i <= n; i += 2){
	   pthread_mutex_lock(&thread_race_lock);
	   while(!check)
		   pthread_cond_wait(&condition_sat, &thread_race_lock);
	   printf("%d\n", i);
	   check = false;
	   pthread_cond_signal(&condition_sat);
	   pthread_mutex_unlock(&thread_race_lock);
   }
...
void print_numbers(int n)
{
    ...
    *arg = n;
    ...
    if (pthread_mutex_init(&thread_race_lock, NULL) != 0){
