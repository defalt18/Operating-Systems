...
void show(char *fname)
{
    ...
    // Check if desired file is a directory or not
    type_check = _inode_table[CD_INODE_ENTRY].TT[0];
    ...
    int blocks[3], i, j, destination_inode = -1;
    blocks[0] = stoi(_inode_table[CD_INODE_ENTRY].XX, 2);
    blocks[1] = stoi(_inode_table[CD_INODE_ENTRY].YY, 2);
    blocks[2] = stoi(_inode_table[CD_INODE_ENTRY].ZZ, 2);
    ...
    for(i=0; i<3; i++)
    {
        ...
        for(j=0; j<4; j++)
        {   
            // If mapping of inode is valid
            if(_directory_entries[j].F == '1')
            {
                // string match only , status of filw type remains unclear
                if(strcmp(_directory_entries[j].fname, fname) == 0)
                {
                    // check for a file or folder
                    type_check = _inode_table[destination_inode].TT[0];
                    
                    if(type_check == 'D')
                        continue;

                    destination_inode = stoi(_directory_entries[j].MMM, 3);
                    break;
                }
            }
    ...
    {
        blocks[0] = stoi(_inode_table[destination_inode].XX, 2);
        blocks[1] = stoi(_inode_table[destination_inode].YY, 2);
        blocks[2] = stoi(_inode_table[destination_inode].ZZ, 2);
...
{
    char type_check;
    int blocks[3], i, j, k, null_block_pos = -1, directory_pos = -1, created_block = 0;
    int n_inode;

    char buffer[1024];
    
    //current directory is a file, some error in logic
    type_check = _inode_table[CD_INODE_ENTRY].TT[0];
    ...
    for(i=0; i<3; i++)
    {
        ...
        for(j=0; j<4; j++)
        {   
            // If mapping turns out to be valid
            if(_directory_entries[j].F == '1')
            {
                // Check if the file or directory with desired name is already present
                if(strcmp(_directory_entries[j].fname, fname) == 0)
                {
                    printf("%s: File or directory already exists.\n", fname);
                    return;
                }
            }
            else
            {
                //If found an empty directory
                null_block_pos = i;
                directory_pos = j;
                created_block = 0;
            }
    ...
    if(null_block_pos == -1 || directory_pos == -1)
    {
        printf("Out of storage space in the directory!\n");
        return;
    }
    else if(created_block)
    {
        // Creation of block if mapping space available
        ...
        if(n_inode == -1)
        {
            ...
            printf("No nodes left to map\n");
            return;
        }
        
        // if the mapping is correct
        switch(null_block_pos)
        {
        ...
        blocks[null_block_pos] = new_block;
    }
    else
    {
        n_inode = getInode();
    ...
    readSFS(blocks[null_block_pos], (char *)new_dir_block);
        
    new_dir_block[directory_pos].F = '1';
    strncpy(new_dir_block[directory_pos].fname, fname, 252);
    itos(new_dir_block[directory_pos].MMM, n_inode, 3);
    
    // Writing blocks to file system
    writeSFS(blocks[null_block_pos], (char *)new_dir_block);
    
    // Registering the inodes in table
    strncpy(_inode_table[n_inode].TT, "FI", 2);
    strncpy(_inode_table[n_inode].XX, "00", 2);
    strncpy(_inode_table[n_inode].YY, "00", 2);
    strncpy(_inode_table[n_inode].ZZ, "00", 2);

    writeSFS(BLOCK_INODE_TABLE, (char *)_inode_table);

    printf("%s created, enter desired text!\n", fname);
    ...
    while(i < 3)
    {
        ...
        switch(i)
        {
            case 0: itos(_inode_table[n_inode].XX, new_block, 2); break;
            case 1: itos(_inode_table[n_inode].YY, new_block, 2); break;
            case 2: itos(_inode_table[n_inode].ZZ, new_block, 2); break;
        }
...
{
    char type_check = _inode_table[inode].TT[0];
    int blocks[3], i;
    if(type_check == 'D')
    {
        printf("error: inode is a directory!\n");
        exit(1);
    }
...
int removeDirectory(int inode)
{
    ...
    for(i=0; i<3; i++)
    {
        ...
        for(j=0; j<4; j++)
        {
            ...
            int delete_node = stoi(directory_entries[j].MMM, 3);
            type_check = _inode_table[delete_node].TT[0];
            if(type_check == 'F')
            {
                if(!removeFile(delete_node))
                {
            ...
            {
                if(!removeDirectory(delete_node))
                {
...
{
    char type_check = _inode_table[CD_INODE_ENTRY].TT[0];
    int blocks[3], i, j, valid_name = 0, items_in_bag = 0;
    
    if(type_check == 'F')
    {
        printf("error: CD_INODE is a file!\n");
        exit(1);
    }
    ...
    for(i=0; i<3; i++)
    {
        ...
        for(j=0; j<4; j++)
        {
            ...
            if(strcmp(fdname, directory_entries[j].fname) == 0)
            {
                ...
                type_check = _inode_table[delete_node].TT[0];
                if(type_check == 'F')
                    removeFile(delete_node);
                else
                    removeDirectory(delete_node);
...
int main()
{
    ...
    printf("Disk sfs mounted \n");
    printf("Operating Instructions : \n 1. All the commands which require a filename to be taken as input needs to be first run individually then filename needs to be input. \n For e.g. \nA cd command will run as follows : \n>cd\n>filename\n-------output--------\n Thank you for reading!\n");

    while(1)
    {
        char command_input[1024], filedesc[1024];
        ...
        // If user wants to list the files in directory
        else if(strcmp(command_input, "ls") == 0)
            ls();

        // If user wants to head over to a directory
        ...
        // user heads over to the highest level of structure
        else if(strcmp(command_input, "rd") == 0)
            rd();

        // show resource statistics
        else if(strcmp(command_input, "stats") == 0)
            stats();

        // user creates a directory with filename <filedesc>
        else if(strcmp(command_input, "md") == 0)
        {
            scanf("%s", filedesc);
            md(filedesc);
        }

        // If user wants opts to display contents of a particular file
        else if(strcmp(command_input, "display") == 0)
        {   
            scanf("%s", filedesc);
            show(filedesc);
        }
        ...
        else if(strcmp(command_input, "create") == 0)
        {   
            scanf("%s", filedesc);
            create(filedesc);
        }
        ...
        else if(strcmp(command_input, "remove") == 0)
        {   
            scanf("%s", filedesc);
            remfd(filedesc);
        }

        // If an invalid command is issued
        else if(strlen(command_input) > 0)
